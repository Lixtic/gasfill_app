...
async def update_order_status(order_id: str, status_update: OrderStatusUpdate):
    ...
    order["updatedAt"] = utc_now().isoformat()
...
async def get_admin_dashboard(current_admin: dict = Depends(get_current_admin)):
    ...
    total_revenue = sum(order["total"] for order in orders_db if order["status"] == "delivered")
    monthly_revenue = sum(
        order["total"] for order in orders_db 
...
async def get_rider_orders(
    ...
    if status:
        rider_orders = [order for order in rider_orders if order["status"] == status]
...
async def get_available_orders(current_rider: dict = Depends(get_current_rider)):
    ...
    # Get unassigned orders in rider's coverage area
    available_orders = [
        order for order in orders_db 
        if order["status"] == "pending" and not order.get("assigned_rider_id")
    ]
...
async def accept_order(order_id: str, current_rider: dict = Depends(get_current_rider)):
    ...
    order["assigned_rider_id"] = current_rider["id"]
    order["assigned_rider_name"] = current_rider["username"]
    order["status"] = "assigned"
    order["assigned_at"] = utc_now().isoformat()
    order["updated_at"] = utc_now().isoformat()
...
async def update_delivery_status(
    ...
    if status_update.notes:
        if "delivery_notes" not in order:
            order["delivery_notes"] = []
        order["delivery_notes"].append({
